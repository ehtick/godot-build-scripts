...
if [ ! -d "deps/moltenvk" ]; then
  ...
  pushd deps/moltenvk
  curl -LO https://github.com/KhronosGroup/MoltenVK/releases/download/v1.4.2/MoltenVK-all.tar
...
if [ ! -d "deps/accesskit" ]; then
  ...
  curl -L -o accesskit.zip https://github.com/godotengine/godot-accesskit-c-static/releases/download/0.22.3/accesskit-c-0.22.3.zip
...
if [ ! -d "deps/mesa" ]; then
  ...
  mkdir -p deps/mesa
  pushd deps/mesa
  base_url=https://github.com/godotengine/godot-nir-static/releases/download/25.3.1-3/godot-nir-static
...
if [ ! -d "deps/perfetto" ]; then
  echo "Missing Perfetto libraries, downloading them."
  mkdir -p deps/perfetto
  pushd deps/perfetto
  curl -L -O https://github.com/google/perfetto/releases/download/v54.0/perfetto-cpp-sdk-src.zip
  unzip -o perfetto-cpp-sdk-src.zip && rm -f perfetto-cpp-sdk-src.zip
  popd
fi
